    ...
    fun testAllTcping() {
        ...
        for (k in 0 until AngConfigManager.configs.vmess.count()) {
            ...
            if (AngConfigManager.configs.vmess[k].configType == EConfigType.CUSTOM.value) {
                val serverOutbound = V2rayConfigUtil.getCustomConfigServerOutbound(
                    getApplication(), AngConfigManager.configs.vmess[k].guid
                ) ?: continue
            ...
            tcpingTestScope.launch {
                AngConfigManager.configs.vmess.getOrNull(k)?.let {
